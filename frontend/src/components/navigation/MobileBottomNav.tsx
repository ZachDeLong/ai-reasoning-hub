...
export const MobileBottomNav: FC<MobileBottomNavProps> = ({ activePage, setActivePage, onOpenFilters }) => {
  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white dark:bg-stone-900 border-t border-stone-200 dark:border-stone-800 safe-area-bottom">
      <div className="flex items-center justify-around h-16 px-2">
        {navItems.map(item => (
          <button
            key={item.id}
            onClick={() => item.id === 'filters' ? onOpenFilters() : setActivePage(item.id as NavItemId)}
